...
export default function DashboardPage() {
  ...
  const pixExpenses = currentMonthExpenses.filter((e) => e.paymentMethod === "pix").reduce((a, e) => a + e.amount, 0)
  const cardExpenses = currentMonthExpenses.filter((e) => e.paymentMethod === "cartao").reduce((a, e) => a + e.amount, 0)
  const fixedExpenses = currentMonthExpenses.filter((e) => e.type === "fixo").reduce((a, e) => a + e.amount, 0)
  const variableExpenses = currentMonthExpenses.filter((e) => e.type === "variavel").reduce((a, e) => a + e.amount, 0)
  ...
  const monthsData = Array.from({ length: 6 }, (_, i) => {
    ...
    const monthExp = expenses.filter((e) => { try { return isWithinInterval(parseISO(e.date), { start: mStart, end: mEnd }) } catch { return false } }).reduce((a, e) => a + e.amount, 0)
    const monthInc = incomes.filter((i) => { try { return isWithinInterval(parseISO(i.date), { start: mStart, end: mEnd }) } catch { return false } }).reduce((a, i) => a + i.amount, 0)
    return { month: format(month, "MMM", { locale: ptBR }), despesas: monthExp, renda: monthInc, saldo: monthInc - monthExp }
  })

  const euExpenses = currentMonthExpenses.filter((e) => e.person === "eu").reduce((a, e) => a + e.amount, 0)
  const parceiroExpenses = currentMonthExpenses.filter((e) => e.person === "parceiro").reduce((a, e) => a + e.amount, 0)
  const personData = [
  ...
  const paymentMethodData = [
    ...
    { name: "Dinheiro", value: currentMonthExpenses.filter((e) => e.paymentMethod === "dinheiro").reduce((a, e) => a + e.amount, 0), color: "#4ade80" },
    { name: "Transferencia", value: currentMonthExpenses.filter((e) => e.paymentMethod === "transferencia").reduce((a, e) => a + e.amount, 0), color: "#166534" },
    { name: "Outro", value: currentMonthExpenses.filter((e) => e.paymentMethod === "outro").reduce((a, e) => a + e.amount, 0), color: "#86efac" },
  ].filter((p) => p.value > 0)
  ...
  const dailySpending = daysInMonth.map((day) => ({
    dia: format(day, "dd"),
    valor: currentMonthExpenses.filter((e) => { try { return isSameDay(parseISO(e.date), day) } catch { return false } }).reduce((a, e) => a + e.amount, 0),
  }))
  ...
  const stats = [
    { label: "Renda Total", value: totalIncomes, icon: TrendingUp, trend: "up" as const, color: "text-primary", bg: "bg-primary/10" },
    { label: "Despesas", value: totalExpenses, icon: TrendingDown, trend: "down" as const, color: "text-destructive", bg: "bg-destructive/10" },
    { label: "Saldo", value: balance, icon: Wallet, trend: balance >= 0 ? "up" as const : "down" as const, color: balance >= 0 ? "text-primary" : "text-destructive", bg: balance >= 0 ? "bg-primary/10" : "bg-destructive/10" },
    { label: "Pix", value: pixExpenses, icon: Smartphone, trend: "neutral" as const, color: "text-muted-foreground", bg: "bg-muted" },
    { label: "Cartao", value: cardExpenses, icon: CreditCard, trend: "neutral" as const, color: "text-muted-foreground", bg: "bg-muted" },
    { label: "Custos Fixos", value: fixedExpenses, icon: PiggyBank, trend: "neutral" as const, color: "text-muted-foreground", bg: "bg-muted" },
  ]
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      {/* Stats Cards - fixed mobile layout */}
                  ...
                  <div>
                    ...
                    <div className="flex flex-col sm:flex-row items-center gap-6">
                      <div className="h-56 w-56 shrink-0">
                        <ResponsiveContainer width="100%" height="100%">
                          <PieChart>
                            <Pie data={categoryData} cx="50%" cy="50%" innerRadius={55} outerRadius={85} paddingAngle={3} dataKey="value" animationBegin={0} animationDuration={800}>
                  ...
                  {paymentMethodData.length > 0 ? (
                    <div className="flex flex-col sm:flex-row items-center gap-6">
                      <div className="h-48 w-48 shrink-0">
                        <ResponsiveContainer width="100%" height="100%">
                          <PieChart>
                            <Pie data={paymentMethodData} cx="50%" cy="50%" outerRadius={75} paddingAngle={2} dataKey="value" animationBegin={0} animationDuration={800}>
                    ...
                    <div className="flex flex-col sm:flex-row items-center gap-6">
                      ...
                      <div className="flex flex-col gap-4 flex-1">
                        <div className="flex flex-col gap-2">
                          ...
                          <span className="text-xs text-muted-foreground">{totalExpenses > 0 ? ((fixedExpenses / totalExpenses) * 100).toFixed(1) : "0"}% do total</span>
                        </div>
                        <div className="flex flex-col gap-2">
                          <div className="flex items-center justify-between">
                            <div className="flex items-center gap-2">
                          ...
                          <span className="text-xs text-muted-foreground">{totalExpenses > 0 ? ((variableExpenses / totalExpenses) * 100).toFixed(1) : "0"}% do total</span>
                        </div>
                      </div>
                    </div>
                  ) : emptyChartMessage}
                    ...
                    <div className="h-72">
                      <ResponsiveContainer width="100%" height="100%">
                        <RadarChart data={radarData}>
                          ...
                          <Radar name={personNames.eu} dataKey={personNames.eu} stroke="#21C25E" fill="#21C25E" fillOpacity={0.3} strokeWidth={2} />
                          <Radar name={personNames.parceiro} dataKey={personNames.parceiro} stroke="#15803d" fill="#15803d" fillOpacity={0.3} strokeWidth={2} />
                          <Legend iconType="circle" iconSize={8} wrapperStyle={{ fontSize: '12px' }} />
                          <Tooltip content={<CustomTooltipContent />} />
                        </RadarChart>
                      </ResponsiveContainer>
            ...
            {personData.length > 0 ? (
              <div className="flex flex-col sm:flex-row items-center gap-6">
                <div className="h-48 w-48 shrink-0">
                  <ResponsiveContainer width="100%" height="100%">
                    <PieChart>
                      <Pie data={personData} cx="50%" cy="50%" innerRadius={45} outerRadius={75} paddingAngle={3} dataKey="value">
